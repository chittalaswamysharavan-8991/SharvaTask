...
for (const viewport of [{name:'desktop',width:1280,height:900},{name:'mobile',width:390,height:844}]) {
 ...
 const notes=page.locator('textarea[data-detail-field="notes"]'); await notes.fill('Browser mounted edit');
 await page.locator('button[data-action="save-detail"]').click(); await page.waitForTimeout(200);
 let calls=await page.evaluate(()=>window.__calls); if(!calls.some(c=>c.name==='edit_task_details')) throw new Error(`${viewport.name}: save tool not called`);
 await page.locator('button[data-action="back-board"]').click();
 await page.locator('button[data-action="refresh"]').first().click(); await page.waitForTimeout(200);
 await page.locator('button[data-action="history"]').first().click(); await page.waitForTimeout(200);
 calls=await page.evaluate(()=>window.__calls);
 for (const required of ['edit_task_details','refresh_board_state','get_history']) if(!calls.some(c=>c.name===required)) throw new Error(`${viewport.name}: ${required} not called`);
 const body=await page.locator('body').innerText(); if(!body.includes('History')) throw new Error(`${viewport.name}: history not rendered`);
