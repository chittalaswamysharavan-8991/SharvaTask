  ...
  for (const event of events) {
    ...
    if (event.action === 'list_created') {
      ...
      lists.set(event.list_id, {
        ...
        title: asString(payload.title, 'Untitled List'),
        project: asString(payload.project, 'General'),
        status: 'active',
    ...
    if (event.action === 'task_added') {
      const item: SharvaTaskItem = {
        item_id: asString(payload.item_id, `TASK-${current.items.length + 1}`),
    ...
    if (event.action === 'task_proof_added') {
      const item = current.items.find((candidate) => candidate.item_id === payload.item_id);
      if (item) {
        const proof = asString(payload.proof, 'Proof added');
